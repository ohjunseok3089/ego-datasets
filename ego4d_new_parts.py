...
def parse_processing_data(raw_data_string):
    ...
    Args:
        raw_data_string (str): The raw text data containing UIDs and time ranges.

    Returns:
        dict: A dictionary where keys are UIDs and values are lists of
    ...
    for line in lines:
        ...
        if not line:
            continue
        # Check if the line is a UID (UUID format)
        if re.match(r'^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$', line):
            current_uid = line
            data[current_uid] = []
        elif current_uid and ('~' in line or 'End' in line):
            # This line is a time range for the current UID
            data[current_uid].append(line)
    return data
...
def split_videos(video_base_path, processing_data, output_folder):
    ...
    for video_uid, time_ranges_with_cat in processing_data.items():
        # Look for video file with this UID
        source_video_path = None
        for ext in ['.mp4', '.MP4']:
            potential_path = os.path.join(video_base_path, f"{video_uid}{ext}")
            if os.path.exists(potential_path):
                source_video_path = potential_path
                break

        if not source_video_path:
            print(f"--- WARNING: Source file not found for UID {video_uid}, skipping ---")
            continue
        ...
        try:
            with VideoFileClip(source_video_path) as clip:
                ...
                fps = clip.fps
                if not fps:
                    print(f"  - WARNING: Could not determine FPS for {video_uid}. Skipping.")
                    continue
                ...
                for time_range_str in time_ranges_with_cat:
                    try:
                        ...
                        transcript_status = match.group(3).strip() if match.group(3) else ""

                        # Handle transcript status in filename
                        if transcript_status and "without transcript" in transcript_status:
                            category += "_no_transcript"

                        # Parse time range
                        time_parts = re.split(r'\s*[~-]\s*', time_part.strip())
                        if len(time_parts) != 2:
                        ...
                        output_filename = f"{base_name}({start_frame}_{end_frame}_{category}).mp4"
                        ...
                        print(f"    - ERROR processing time range '{time_range_str}' for {video_uid}: {e}")
                        continue

        except Exception as e:
            print(f"--- ERROR: Failed to process video file {video_uid}: {e} ---")
            continue
...
if __name__ == "__main__":
    ...
    output_path = "/mas/robots/prg-ego4d/raw/new_parts/"
    ...
    42:20 ~ End (collaborative_task) (without transcript)
    ...
    47:35 ~ End (collaborative_task) (without transcript)
    
    a223fcb2-8ffa-4826-bd0c-91027cf1c11e
    0:23 ~ 5:13 (collaborative_task)
    5:13 ~ 19:50 (collaborative_task) (without transcript)
    19:50 ~ 50:12 (collaborative_task)
    50:12 ~ 1:00:12 (collaborative_task) (without transcript)
    
    b3937482-c973-4263-957d-1d5366329dad
    2:19 ~ 5:55 (collaborative_task)
    5:55 ~ 15:53 (collaborative_task) (without transcript)
    15:53 ~ 35:53 (collaborative_task)
    35:53 ~ 40:53 (collaborative_task) (without transcript)
    40:53 ~ 45:50 (collaborative_task)
    45:50 ~ 50:50 (collaborative_task) (without transcript)
    ...
    print("Parsed video processing data:")
    for uid, ranges in video_processing_data.items():
        print(f"  {uid}: {len(ranges)} time ranges")
