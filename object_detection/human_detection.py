...
def is_face_inside_body(face_box, body_box):
    ...
    # MODIFIED: Changed the upper body limit from 0.6 to 0.8 to be more lenient for seated/leaning people.
    upper_body_limit = body_y1 + (body_height * 0.8)
        ...
        if frame_number in frame_detections:
            for detection in frame_detections[frame_number]:
                ...
                if not person_label.startswith('person_id_') and not person_label.startswith('person_no_id'):
                    ...
                    confidence = float(detection['confidence']) # Ensure confidence is float

                    cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 255, 0), 2)
                    # MODIFIED: Changed label format to remove decimal points.
                    label = f"{person_label} {int(confidence * 100)}"
...
def process_video_with_yolo(video_path, output_video_path, output_csv_path, face_csv_path=None):
    ...
    while cap.isOpened():
        ...
        # MODIFIED: Lowered confidence threshold from 0.5 to 0.4
        results = model.track(frame, classes=0, conf=0.4, verbose=False, persist=True, tracker="bytetrack.yaml")
